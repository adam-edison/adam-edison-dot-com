...
const EmailServiceConfigurationSchema = z.object({
  RESEND_API_KEY: z.string({ required_error: 'RESEND_API_KEY is not configured' }).min(1, 'RESEND_API_KEY is not configured'),
  ...
  EMAIL_SENDER_NAME: z.string({ required_error: 'EMAIL_SENDER_NAME is not configured' }).min(1, 'EMAIL_SENDER_NAME is not configured'),
  EMAIL_RECIPIENT_NAME: z.string({ required_error: 'EMAIL_RECIPIENT_NAME is not configured' }).min(1, 'EMAIL_RECIPIENT_NAME is not configured'),
  SEND_EMAIL_ENABLED: z.string({ required_error: 'SEND_EMAIL_ENABLED must be set to "true"' }).refine((val) => val === 'true', 'SEND_EMAIL_ENABLED must be set to "true"')
});
